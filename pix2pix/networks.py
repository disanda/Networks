...
#调整学习率learning_rate的一个类,前期学习率大，后期学习率小，训练更稳定
...
def get_norm_layer(norm_type='instance'):
    ...
    return norm_layer
#instance正则化层，functools.partial(a,b)可以简化函数a
...
#__init__部分为网络的零件，forward()部分为前向传播就是网络的具体结构
...
class Inconv(nn.Module):
    def __init__(self, in_ch, out_ch, norm_layer, use_bias):
        ...
        self.inconv = nn.Sequential(
            ...
            #这个是镜像填充，通过矩阵中的对称值扩充矩阵的边,扩充3行3列
...
class ResBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
#两个填充方法:nn.ReflectionPad2d,nn.ReplicationPad2d
        conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias),
                       norm_layer(dim)]
...
#PatchGAN,返回(1,1,32,32)，即一个32*32的矩阵指导D，而不是bool值
class GANLoss(nn.Module):
    def __init__(self, use_lsgan=True, target_real_label=1.0, target_fake_label=0.0):
        super(GANLoss, self).__init__()
        self.register_buffer('real_label', torch.tensor(target_real_label))
        #这个用于保存一个不需要梯度更新的参数 real_label=1,fake_label=0
    ...
    def get_target_tensor(self, input, target_is_real):
        ...
        #把target_tensor扩充维度为input的维度
    def __call__(self, input, target_is_real):
    #__call__函数可以让类的实例像函数那样调用
